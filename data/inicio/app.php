<?php 
	...
	if (isset($_POST['cargar_facturas_compra'])) {
		$resultado = $class->consulta("SELECT SUM(CAST(total_compra as float)) total_compra FROM factura_compra WHERE fecha_emision = '$fecha_corta' AND estado = '1' ORDER BY total_compra DESC");
		while ($row = $class->fetch_array($resultado)) {
	...
	if (isset($_POST['cargar_facturas_venta'])) {
		$resultado = $class->consulta("SELECT SUM(CAST(total_venta as float)) total_venta FROM factura_venta WHERE fecha_emision = '$fecha_corta' AND estado = '1' ORDER BY total_venta DESC");
		while ($row = $class->fetch_array($resultado)) {
