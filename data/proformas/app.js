app.controller('proformasController', function ($scope, $interval) {
	...
	jQuery(function($) {
	    ...
	    jQuery(grid_selector2).jqGrid({	 
	        ...
	        colNames: ['ID','IDENTIFICACIÓN','RAZÒN SOCIAL','DIRECCIÓN','FECHA EMISIÓN','TOTAL','PDF','COMPARTIR'],
	        colModel:[ 
			    {name:'id',index:'id', frozen:true, align:'left', search:false, hidden: true},   
	        ...
	        gridComplete: function() {
				...
				for(var i = 0;i < ids.length;i++) {
					...
					share = "<a onclick=\"angular.element(this).scope().methodoshare('"+id_proforma+"')\" title='Compartir Proforma' data-toggle='modal' data-target='#myModal2' ><i class='fa fa-share-alt' style='cursor:pointer; cursor: hand'> SHARE</i></a>"; 					
	        ...
	        ondblClickRow: function(rowid) {     	            	            
            	...
            	$.ajax({
					...
					success: function(data) {
						...
						if(data.estado == "0") {
							$("#estado h3").remove();
	                        $("#estado").append($("<h3>").text("Procesado"));
	                        $("#estado h3").css("color","red");
	                        $("#btn_2").attr("disabled", true);
	                    } else {
	                        $("#estado h3").remove();
	                        $("#estado").append($("<h3>").text("Pendiente"));
	                        $("#estado h3").css("color","green");
