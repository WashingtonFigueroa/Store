...
class PDF extends PDF_Rotate {
    ...
    $resultado = $class->consulta("SELECT C.razon_social, C.identificacion, C.direccion, C.telefono2, C.ciudad, F.fecha_emision, P.nombre_forma, F.tarifa, F.tarifa0, F.iva, F.total_descuento, F.total_venta, F.efectivo, F.cambio, F.estado FROM factura_venta F, clientes C, formas_pago P WHERE F.id_cliente = C.id AND F.id_forma_pago = P.id AND F.id = '$_GET[id]'");    
    while ($row = $class->fetch_array($resultado)) {        
        $pdf->SetFont('Arial','',9);	   
        $pdf->Text(25, 37, utf8_decode('' . strtoupper($row[0])), 0, 'C', 0); ////CLIENTE (X,Y)   
        $pdf->Text(25, 43, utf8_decode('' . strtoupper($row[1])), 0, 'C', 0); ///FECHA (X,Y)
        $pdf->Text(25, 49, utf8_decode('' . strtoupper($row[2])), 0, 'C', 0); ///DIRECCION(X,Y)  
        $pdf->Text(155, 43, utf8_decode('' . strtoupper($row[5])), 0, 'C', 0); ///RUC CI(X,Y)    
