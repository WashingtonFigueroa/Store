<?php
    ...
    while ($row = $class->fetch_array($resultado)) {
        $resultado1 = $class->consulta("SELECT F.serie, F.fecha_actual, F.hora_actual, F.fecha_cancelacion, F.autorizacion, F.id_forma_pago,tarifa0, F.tarifa, F.iva, F.total_descuento, F.total_compra, P.razon_social, P.identificacion, F.id FROM factura_compra F, proveedores P WHERE F.id_proveedor = P.id AND F.id_proveedor = '$row[0]' and fecha_actual between '$_GET[inicio]' and '$_GET[fin]' order by F.id");
