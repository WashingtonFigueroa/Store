<?php
    ...
    $repetido = 0;   
    $contador = 0; 
    $resultado = $class->consulta("SELECT id, identificacion, razon_social FROM clientes");
    while ($row = $class->fetch_array($resultado)) {
        $repetido = 0;   
        $contador = 0; 
        $total=0;
        $resultado1 = $class->consulta("SELECT * FROM factura_venta WHERE fecha_emision between '$_GET[inicio]' and '$_GET[fin]' and id_cliente = '$row[0]' and estado = '1'");
