<?php
    ...
    while ($row = $class->fetch_array($resultado)) {        
        $resultado1 = $class->consulta("SELECT F.secuencial, F.fecha_emision, F.tipo_precio, F.id_forma_pago, F.tarifa0, F.tarifa, F.iva, F.total_descuento, F.total_venta, C.identificacion, C.razon_social, F.id FROM factura_venta F, clientes C WHERE F.id_cliente = C.id AND F.id_cliente = '$row[0]' and fecha_emision between '$_GET[inicio]' and '$_GET[fin]' order by F.id asc");                        
